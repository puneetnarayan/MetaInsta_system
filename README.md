...
- **Help** tab: what each tab does, what to watch out for, the recommended order to work through them, and a "Go to tab" link on every entry that jumps straight there
- **Grouped navigation**: tabs are grouped into Plan / Create / Launch / Analyse / Tools, matching the roadmap's suggested structure, instead of one long unsorted row
- **Colour system**: an orange accent (buttons/links/selected states) with three-level depth — base, a darker hover, and a darkest "selected/active" state — plus field-level colour coding: light orange for fields you need to fill in, light yellow for generated/heuristic fields you're free to edit, deepening to a stronger yellow while you're actively editing one (see the Help tab's "Field colours" note). Each nav group (Plan/Create/Launch/Analyse/Tools) also carries its own pastel identity — a distinct header colour and card tint per group (blue/purple/green/teal/rose) — so it's visually obvious which stage of the workflow a tab belongs to, independent of the orange action colour
